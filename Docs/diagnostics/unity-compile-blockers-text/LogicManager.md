...
/// <summary>
/// Gestor de lÃ³Â·Â´Â·gica principal del juego.
/// Implementa m Ã¡â€”quina de estados con validaciones de seguridad.
/// </summary>
public class LogicManager : MonoBehaviour, ILogicManager
{
    ...
    public static LogicManager Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                if (_instance == null)
                {
                    Debug.LogError("[LogicManager] No se encontrÃ³ LogicManager en la escena!");
                }
            }
    ...
    void Update()
    {
        if (!isInitialized)
        {
            Debug.LogWarning("[LogicManager] Update() llamado pero no estÃ¡ inicializado");
            return;
        }
    ...
    public void Initialize(IScreenManager sm, IInputManager im, ISpriteDatabase sd)
    {
        if (isInitialized)
        {
            Debug.LogWarning("[LogicManager] Ya estÃ¡ inicializado");
            return;
        }
    ...
    public void ChangeState(GameState newState)
    {
        if (!isInitialized)
        {
            Debug.LogWarning("[LogicManager] ChangeState() llamado pero no estÃ¡ inicializado");
            return;
        }
    ...
    private void SafeChangeState(GameState newState)
    {
        try
        {
            // VALIDACIÃ“Â·N CR Ãâ€”TICA #1: Verificar que el estado existe
            if (states == null)
            {
            ...
            // VALIDACIÃ“Â·N CR Ãâ€”TICA #2: Verificar que el estado no es null
            ...
            // VALIDACIÃ“Â·N CR Ãâ€”TICA #3: Prevenir cambio al mismo estado
        ...
        {
            Debug.LogError($"[LogicManager] ExcepciÃ³Â·Â´Â·n en SafeChangeState: {ex.Message}");
            Debug.LogError($"[LogicManager] Stack: {ex.StackTrace}");
    ...
    {
        Debug.Log($"[LogicManager] Cambiando estado: {currentState?.GetType().Name ?? "null"} â†’ {newState}");
        ...
        try
        {
            ...
            {
                Debug.LogError($"[LogicManager] Â¡ERROR CR Ãâ€”TICO! currentState es null despuÃ©s del cambio!");
                throw new InvalidOperationException("currentState es null despuÃ©s del cambio");
            }
    ...
    public void ProcessInput(PlayerInput input)
    {
        if (!isInitialized)
        {
            Debug.LogWarning("[LogicManager] ProcessInput() llamado pero no estÃ¡ inicializado");
            return;
        }
    ...
    #region Utility
    /// <summary>
    /// Verifica si un estado estÃ¡ registrado.
    /// </summary>
